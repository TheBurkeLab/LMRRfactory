...
def plot_ratefit(temperatures, rate_constants, pltcolour, labell,idx): #3-parameter fit (A, b, Ea)
    ...
    print(f"{labell}   A: {A_fit}, b: {beta_fit}, Ea: {Ea_fit}")
...
print("\n\n\n")
T_list=[300,1000,2000]
plot_ratefit(np.array([300, 1000, 2000]), np.array([11.2/3.15,13.4/2.47,14.3/2.25]), pltcolours[2], "CO$_2$",idx)
plot_ratefit(np.array([300, 1000, 2000]), np.array([14.0/3.15,23.6/2.47,27.9/2.25]), pltcolours[4], "H$_2$O",idx)
plot_ratefit(np.array([300, 1000, 2000]), np.array([1/3.15,1/2.47,1/2.25]), pltcolours[0], "AR",idx)
plot_ratefit(np.array([300, 1000, 2000]), np.array([13.9/3.15,20.0/2.47,22.2/2.25]), pltcolours[3], "NH$_3$",idx)
plot_ratefit(np.array([300, 1000, 2000]), np.array([1.55/3.15,1.48/2.47,1.70/2.25]), pltcolours[1], "O$_2$",idx)
...
fig.text(0.5, 0.07, r'Temperature [K]', ha='center', va='center',fontsize=7)
name = f'allArrheniusFits'
if save_plots == True:
    plt.savefig('manual.pdf', dpi=500, bbox_inches='tight')
    plt.savefig('manual.svg', dpi=500, bbox_inches='tight')